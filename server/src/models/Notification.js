...
const notificationSchema = mongoose.Schema(
  {
    ...
    postId: { type: mongoose.Schema.Types.ObjectId, ref: 'Post' }, // 게시글 ID
    readAt: Date, // 읽은 시간
...
notificationSchema.statics.registerNotification = async function (postId, targetUserId, generateUserId, noticeType, generateObjectId) {
  const isNoticeExist = await Notification.findOne({ postId: postId, generateObjectId: generateObjectId })
  if (!isNoticeExist && targetUserId != generateUserId) {
    let noticeCode = noticeType == 'like' ? '0' : noticeType == 'comment' ? '1' : noticeType == 'reply' ? '2' : ''
    await Notification.create({ targetUserId, generateUserId, postId, noticeCode, noticeType, generateObjectId })
  }
}
